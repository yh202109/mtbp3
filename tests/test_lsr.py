...
class TestLsrTree(unittest.TestCase):
    ...
    def test_list_files_list(self):
        ...
        expected_files = ['/testfolder1/testfile11', '/testfolder2/testfile21', '/testfolder2/testfile3']
        self.assertCountEqual(files, expected_files)

    def test_list_files_json(self):
        lsr = LsrTree(self.test_folder, outfmt="json")
        files = lsr.list_files()
        expected_files = '{"0": {"path": "", "level": 0, "folders": ["testfolder1", "testfolder2"], "files": []}, "1": {"path": "/testfolder1", "level": 1, "folders": [], "files": ["testfile11"]}, "2": {"path": "/testfolder2", "level": 1, "folders": [], "files": ["testfile21", "testfile3"]}}'
        self.assertCountEqual(files, expected_files)

    def test_list_files_dataframe(self):
        lsr = LsrTree(self.test_folder, outfmt="dataframe")
        files = lsr.list_files()['file'].tolist()
        expected_files = ['testfile11', 'testfile21', 'testfile3']
        self.assertCountEqual(files, expected_files)
